...
var LOGIC = {
  ...
  init: function () {
    ...
    this.chat_button.addEventListener("click", this.showChat.bind(this));
    this.send_button.addEventListener("click", this.processInput.bind(this));

    this.emoji_button.addEventListener(
      "click",
      this.displayEmojiTable.bind(this)
    );
  ...
  updateUserInput: function (dt, user) {
    ...
    if (user.target_position[0] - user.position[0] < 0) {
      ...
      if (diff > 1 && user.position[0] >= GFX.min_room_pos) {
        user.position[0] = this.lerp(
          user.position[0],
          user.target_position[0],
          0.05
        );
        user.position[1] = map_range(
          user.position[0],
          -100,
          100,
          0,
  ...
  inputAddEmoji: function () {
    for (let i = 0; i < this.emojis.length; i++) {
      this.emojis[i].addEventListener(
        "click",
        function () {
          this.input_text.value =
            this.input_text.value + this.emojis[i].innerHTML;
        }.bind(this)
      );
    }
  },
  ...
  checkObjects: function (mouse_x, mouse_y) {
    ...
    for (var i = 0; i < room.sprites.length; i++) {
      ...
      if (
        this.checkObjectConditions(centerx, mouse_x, mouse_y, sprite, "door")
      ) {
      ...
      if (
        this.checkObjectConditions(centerx, mouse_x, mouse_y, sprite, "lampara")
      ) {
        this.changeObject(sprite);
        return true;
      }
  ...
  checkObjectConditions: function (centerx, mouse_x, mouse_y, sprite, type) {
    if (sprite.type == type) {
      //esto se puede reutilizar con los diferentes items
      var object = sprite;
      var img = IMAGES[object.src];
      var w = img.width;
      var h = img.height;
      if (
        mouse_x >= object.x + centerx &&
        mouse_x <= object.x + w + centerx &&
        mouse_y >= object.y &&
        mouse_y <= object.y + h
      ) {
        return true;
